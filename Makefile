...
$(UNIT_TESTS): build/unit-test.exe

$(BUILD_DIR)/unit-test.exe: $(BUILD_GTEST)/gtest_main.a build/app-test.o tests/unit/unit-test.cpp
	@$(CXX) -isystem $(GTEST_DIR)/include -pthread \
		tests/unit/unit-test.cpp \
		$(BUILD_GTEST)/gtest_main.a build/app-test.o \
		-o build/unit-test.exe
